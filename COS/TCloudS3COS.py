...
def ReadYaml(YamlPath: str, MainNode: str ):
    ...
    try:
        ...
        if os.path.exists(YamlPath):
            ...
            # must specify schema : https or http
            s3_url="https://"+config[MainNode]['endpoint']
            aws_access_key_id=config[MainNode]['SecretId']
            aws_secret_access_key=config[MainNode]['SecretKey']
            return s3_url,aws_access_key_id,aws_secret_access_key
        else:
            print('Resolve filepath error. Please check !')
            sys.exit(1)
    except Exception as err:
        print('Requests error. Please check !', err)
        sys.exit(1)
...
class S3SdkClient():
    ...
    def CreateS3Clinet(self):
        S3Client = boto3.client('s3', aws_access_key_id=self.aws_access_key_id, aws_secret_access_key=self.aws_secret_access_key,endpoint_url=self.s3_url,use_ssl=True)
        return S3Client
    # Create S3 Bucket
...
if __name__=="__main__":
    s3_url, aws_access_key_id, aws_secret_access_key = ReadYaml("./TCloudCOS.yaml","cos")
